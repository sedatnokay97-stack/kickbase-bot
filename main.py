...
def main():
    print("▶️ Starte finalen Transfermarkt-Abruf...")
    headers = {
    ...
    try:
        ...
        # Liga ID & Name auslesen (Mit den neuen V4 Etiketten!)
        ligen = login_res.get("srvl", [])
        if not ligen:
            print("❌ Keine Ligen gefunden!")
            return
            
        liga_id = ligen[0].get("id")
        liga_name = ligen[0].get("name", "Unbekannt")
        print(f"✅ Eingeloggt! Lade Markt für Liga: {liga_name}")
        
        # 2. Transfermarkt abrufen (Die richtige V4 Adresse)
        ...
        # Spielerliste auslesen (Das Geheim-Etikett "it")
        ...
        msg += "Hier sind 5 Spieler zur Probe:\n"
